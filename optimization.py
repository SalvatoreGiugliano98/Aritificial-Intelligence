...
def randomoptimize(domain,costf):
  ...
  for i in range(0,1000):
    ...
    r=[random.randint(domain[i][0],domain[i][1]) for i in range(int(len(domain)))]
...
def hillclimb(domain,costf):
  # Create a random solution
  sol=[random.randint(domain[i][0],domain[i][1]) for i in range(int(len(domain)))]
  # Main loop
  while 1:
...
def geneticoptimize(domain,costf,popsize=50,step=1,
  ...
  for i in range(popsize):
    vec=[random.randint(domain[i][0],domain[i][1]) for i in range(int(len(domain)))]
    pop.append(vec)
